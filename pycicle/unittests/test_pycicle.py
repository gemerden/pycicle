...
class TestArgParser(unittest.TestCase):
    ...
    def test_basic_keywords(self):
        ...
        class Parser(ArgParser):
            ...
            with_callback = Argument(
                int, callback=lambda v, ns: callback_target.extend([v, ns])
            )

        asserter = args_asserter(
            pos=1, default=0, required=3, valid=4, many=[1, 2], with_callback=5
        )
        ...
        for kwargs in dict_product(
            ...
            assert parser._command(short=True) == make_test_command(
                Parser, kwargs, short=True
            )
    ...
    def test_datetime_types_and_defaults(self):
        ...
        class Parser(ArgParser):
            ...
            timedelta_ = Argument(
                timedelta, default=timedelta(hours=1, minutes=2, seconds=3)
            )
        ...
        parser = Parser(test_cmd, target=asserter)
        real_cmd = parser._command()
        assert test_cmd == real_cmd
        parser2 = Parser(use_gui=False, target=asserter)
    ...
    def test_bool(self):
        ...
        assert_product(Parser, one=(False, True), two=([False, False], [True, False]))
...
class TestDescriptorConfig(unittest.TestCase):
    ...
    def test_not_many_and_no_types(self):
        for kwargs in dict_product(
            ...
            many=False,
            default=(None, 0, 1),
            novalue=(MISSING, None, 2, 3),
            positional=(False, True),
    ...
    def test_many_and_no_types(self):
        for kwargs in dict_product(
            ...
            required=(False, True),
            many=(True, 2),
            default=(None, [0, 1], [2, 3]),
            novalue=(MISSING, None, [3, 4], [4, 5]),
            positional=(False, True),
