...
class Kwargs(object):
    ...
    @classmethod
    def _usage(cls):
        return f"{get_entry_file(path=False)} {' '.join(arg.usage() for arg in cls._arguments.values())}"

    @classmethod
    def _options(cls):
        return '\n  '.join(arg.option() for arg in cls._arguments.values())
...
class CmdParser(object):
    ...
    def cmd_line_help(cls):
        ...
        return f"usage: {cls.kwargs_class._usage()}\n\noptions:\n  {cls.kwargs_class._options()}"
