...
def show_multi_choice_dialog(win, title, chosen, xy, wh=None):
    ...
    dialog.grab_set()
    chosen_vars = {
        choice: tk.BooleanVar(value=boolean) for choice, boolean in chosen.items()
    }
...
class TkArgWrapper(object):
    ...
    def reset_value(self):
        delattr(self.app.parser, self.argument.name)
        if self.argument.novalue is not MISSING:
    ...
    def _get_help_widget(self, master, **kwargs):
        def show():
            ...
            help_text = get_argument_help(
                self.argument, error=self.error, separator=short_line
            )
            show_text_dialog(
                self.app.master,
                title=f"help: {self.argument.name}",
                text=help_text,
                wh=(w, h),
                xy=(x, y),
            )
    ...
    def _get_file_value_widget(self, master, **kwargs):
        self.variable = tk.StringVar(value=self.get_value())
        return self._get_dialog_value_widget(
            master, command=self._open_file_dialog, **kwargs
        )

    def _get_folder_value_widget(self, master, **kwargs):
        self.variable = tk.StringVar(value=self.get_value())
        return self._get_dialog_value_widget(
            master, command=self._open_folder_dialog, **kwargs
        )
    ...
    def _get_boolean_value_widget(self, master, **kwargs):
        if self.argument.many is not False:
            return self._get_multi_choice_value_widget(master, **kwargs)
        return self._get_base_choice_value_widget(
            master, values=["no", "yes"], **kwargs
        )
    ...
    def _get_multi_choice_value_widget(self, master, **kwargs):
        ...
        def show():
            ...
            show_multi_choice_dialog(
                self.app.master, title=f"{self.argument.name}", chosen=chosen, xy=(x, y)
            )
            self.variable.set(
                ", ".join(choice for choice, boolean in chosen.items() if boolean)
            )
            self.app.synchronize()
...
class FormFrame(BaseFrame):
    ...
    column_grid_kwargs = {
        "name": {"sticky": tk.E},  # overrides grid_kwargs
        "value": {"sticky": tk.EW},
        "many": {"sticky": None},
    }
    ...
    def _init(self):
        self.wrappers = [
            TkArgWrapper(app=self.master, argument=arg) for arg in self.master.arguments
        ]
    ...
    def create_widgets(self):
        for i, col_name in enumerate(self.column_names):
            self._create_widget(col_name).grid(
                row=0, column=i, **self._get_grid_kwargs(col_name)
            )
        for i, wrapper in enumerate(self.wrappers):
            for j, col_name in enumerate(self.column_names):
                self._create_widget(col_name, wrapper).grid(
                    row=i + 1, column=j, **self._get_grid_kwargs(col_name)
                )
    ...
    def _create_widget(self, col_name, wrapper=None):
        ...
        return wrapper.create_widget(
            self, name=col_name, **self._get_cell_kwargs(col_name)
        )

    def _create_command_bar(self):
        self.cmd_button = tk.Button(
            self, text="-/--", command=self.switch_command, font=self.bold_font
        )
        ...
        self.cmd_view.grid(
            row=len(self.wrappers) + 1, column=1, columnspan=3, sticky=tk.EW
        )
    ...
    def run(self):
        if self.target is None:
            tk.messagebox.showinfo(
                "nothing to run", "no runnable target was configured for this app"
            )
        if self.synchronize():
            try:
